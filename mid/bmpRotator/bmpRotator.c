...
int main() {
    ...
    header2 = header;
    header2.height =
        ceil(abs(header.height * cos(rad)) + abs(header.width * sin(rad)));
    header2.width =
        ceil(abs(header.height * sin(rad)) + abs(header.width * cos(rad)));
